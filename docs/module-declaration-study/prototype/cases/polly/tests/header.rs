...
/// The store form: the frozen module record semio-db serves and accepts.
#[test]
fn the_declaration_is_also_the_store_record() {
  ...
  let record = polly::record(folder);
  assert_eq!(record.parent, folder);
  assert_eq!(record.name, "polly");
  assert_eq!(record.exports.len(), 2);
  let ExportKind::Function(say) = &record.exports[&polly::ids::say::FUNCTION].kind;
  assert_eq!(say.parameter_ordering, vec![polly::ids::say::TEXT]);
  // One dependency: Status, at the version the type declares.
  assert_eq!(record.dependencies.len(), 1);
  assert_eq!(
    record.dependencies[0].id,
    case_polly::Status::arora_type_id()
  );
  assert_eq!(record.dependencies[0].version.to_string(), "1.0.0");
  // It serializes in the store's wire form and reads back identical.
  let yaml = serde_yaml::to_string(&record).expect("serialize the record");
  let back: Module = serde_yaml::from_str(&yaml).expect("parse the record");
