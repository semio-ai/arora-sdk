...
//! SCHEMA_VERSION, record.rs:156) and into the `*Reference` structs. Here we
//! split that into a reusable trait so an arbitrary type can declare "I have an
//! identity and a version, and here is how two versions of me relate".
...
///
/// Implementing this for an arora type (a module header, a structure, an
/// enumeration, a behavior-tree type, ...) is enough to get version tagging and
/// compatibility checks for free, independent of the freezing machinery.
pub trait Versioned {
  fn id(&self) -> Uuid;
