...
use arora_schema::module::{
  ...
  low::{
    Executor as LowExecutor, ExportFunction as LowExportFunction, ExportSymbol as LowExportSymbol,
    Header as LowHeader, ImportFunction as LowImportFunction, ImportSymbol as LowImportSymbol,
...
pub async fn resolve_type_ref(
  ...
  match type_ref {
    HighTypeRef::Scalar { id } => Ok(LowTypeRef::Scalar {
      id: resolve_type_id(id, registry).await?,
    }),
    HighTypeRef::Array { id } => Ok(LowTypeRef::Array {
      id: resolve_type_id(id, registry).await?,
    }),
...
pub async fn resolve_import_symbol(
  symbol: HighImportSymbol,
  registry: &mut Registry,
) -> anyhow::Result<LowImportSymbol> {
  Ok(match symbol {
...
pub async fn resolve_export_symbol(
  symbol: HighExportSymbol,
  registry: &mut Registry,
) -> anyhow::Result<LowExportSymbol> {
  Ok(match symbol {
...
pub async fn resolve_module_header(
  module_definition: HighModuleDefinition,
  registry: &mut Registry,
) -> anyhow::Result<LowHeader> {
