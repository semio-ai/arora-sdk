...
pub mod tests {
    ...
    fn status_identity_update(engine: &mut PinnedEngine, index: &Index) -> Result<()> {
        ...
        let mut runtime =
            BehaviorTreeRuntime::setup(&behavior, index.clone(), engine, true).unwrap();
    ...
    async fn load_module<R: ReadableRegistry + EditableRegistry + Freezer>(
        ...
        // Find the executable
        let module_path =
            if name == "behavior-tree-nodes" && header.executor.name.as_str() == "wasm" {
    ...
    async fn read_header_from_module_root(module_root: PathBuf) -> Header {
        ...
        let header_path = module_root
            ...
            .join("module.yaml");
        let header: Header =
            serde_yaml::from_str(&read_to_string(header_path.clone()).await.expect(
                format!("header file {} could not be read", header_path.display()).as_str(),
            ))
            .expect(
                format!(
