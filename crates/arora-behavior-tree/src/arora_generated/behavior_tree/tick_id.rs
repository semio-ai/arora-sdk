...
impl Into<Value> for TickId {
    fn into(self) -> Value {
        Value::Structure(Structure {
            id: Uuid::from_bytes(TICK_ID_STRUCT_RAW_ID),
            fields: vec![StructureField {
                id: Uuid::from_bytes(TICK_ID_CALLABLE_ID_FIELD_RAW_ID),
                value: Box::new(Value::U64(self.callable_id)),
            }],
        })
    }
}
impl TryFrom<Value> for TickId {
    type Error = ConversionError;
    fn try_from(value: Value) -> Result<Self, Self::Error> {
        if let Value::Structure(as_struct) = value {
            if *as_struct.id.as_bytes() != TICK_ID_STRUCT_RAW_ID {
                return Err(ConversionError {
                    message: "unexpected structure type ID".to_string(),
                });
            }
            let mut tick_id_callable_id: Option<u64> = None;
            for field in as_struct.fields {
                match *field.id.as_bytes() {
                    TICK_ID_CALLABLE_ID_FIELD_RAW_ID => {
                        tick_id_callable_id = Some(match *field.value {
                            Value::U64(__v) => __v,
                            _ => {
                                return Err(ConversionError {
                                    message: "field callable_id: unexpected value kind".to_string(),
                                })
                            }
                        });
                    }
                    _ => {
                        return Err(ConversionError {
                            message: "unexpected struct field".to_string(),
                        })
                    }
                }
            }
            Ok(TickId {
                callable_id: tick_id_callable_id.ok_or_else(|| ConversionError {
                    message: "missing field callable_id".to_string(),
                })?,
            })
        } else {
            Err(ConversionError {
