...
use ast::{
  ...
  Namespace, NewLine, PreprocessorDirective, Statement, TranslationUnit, TypeRef, Variable,
};
        ...
        for parameter_id in &func.parameter_ordering {
          ...
          function_declarations.push(
            declare::arora_buffer_writer_add_structure_field(
              id::parameter_uuid(&import_asset.import.name, &parameter.name).to_expression(),
            )
            .into(),
          );
          function_declarations.push(
      ...
      ExportKind::Function(f) => {
        ...
        for parameter_id in f.parameter_ordering.iter() {
          ...
          field_declarations.push(
            name
              .assign(declare::deserialize(&type_name))
              .into_statement()
              .into(),
          );
