...
async fn generate_module<'a>(context: &Context<'a>, id: &Uuid) -> anyhow::Result<Arc<Directory>> {
  ...
  source_declarations
    .push(declare::uuid_variable(id::module_uuid(&header.name), &header.id).into());
  ...
  for import in imports {
    match import {
      ImportSymbol::Function(func) => {
        source_declarations
          .push(declare::uuid_variable(id::function_uuid(&func.name), &func.id).into());
        for parameter in func.parameters.iter() {
          source_declarations.push(
            declare::uuid_variable(
              id::parameter_uuid(&func.name, &parameter.name),
              &parameter.id,
            )
            .into(),
          );
        ...
        for parameter in sorted_parameters {
          function_declarations.push(
            declare::arora_buffer_writer_add_structure_field(
              id::parameter_uuid(&func.name, &parameter.name).to_expression(),
            )
            .into(),
          );
          function_declarations.push(
...
fn generate_self_source<'a>(context: &Context<'a>, id: &Uuid) -> anyhow::Result<TranslationUnit> {
  ...
  let mut declarations = Vec::new();
  declarations.push(
    PreprocessorDirective::Include(format!("{}.hpp", &header.name), IncludeStyle::Local).into(),
  );
  declarations
    ...
    match asset {
      ...
      Asset::ImportSymbol(symbol) => {
        if let Some(existing) = context.grouped_import_symbols.get_mut(symbol.module()) {
          ...
          context
            .grouped_import_symbols
            .insert(symbol.module().clone(), vec![symbol]);
        }
      }
